  ...
  if (last16thNoteDrawn != currentNote) {    
    ...
    for (let i = 0; i < metronome.againstBeat; i++) {
      ...
      if (view.selectedGUI === 'view-square' || view.selectedGUI === 'view-pipelines') {
        ...
        canvasContext.rect( AGAINST_BEAT_GRID_X, AGAINST_BEAT_GRID_Y, AGAINST_BEAT_GRID_WIDTH, ELEMENT_BASE_SIZE / 2 );
    ...
    for (let j = 0; j < metronome.baseBeat; j++) {
      ...
      const BASE_BEAT_GRID_X = ELEMENT_BASE_SIZE + (( j * GRID_CELL_SIZE ) / metronome.baseBeat * metronome.againstBeat );
      const BASE_BEAT_GRID_WIDTH = GRID_CELL_SIZE / metronome.baseBeat * metronome.againstBeat ;
      ...
      if (view.selectedGUI === 'view-square' || view.selectedGUI === 'view-pipelines') {
        ...
        canvasContext.beginPath();
        canvasContext.rect( BASE_BEAT_GRID_X, ELEMENT_BASE_SIZE, BASE_BEAT_GRID_WIDTH, ELEMENT_BASE_SIZE / 2 );
        canvasContext.strokeStyle = GRID_BORDER_COLOR;
        canvasContext.fill();
        canvasContext.stroke();
      } else {
